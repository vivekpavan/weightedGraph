...
public class weightedGraph {
    ...
    private class nodeEnter {
        private Node node;
        private int priority;

        public nodeEnter(Node node, int priority) {
            this.node = node;
    ...
    public Path getShortestPath(String from,String to){
        ...
        PriorityQueue<nodeEnter> queue = new PriorityQueue<>(
                Comparator.comparingInt(n -> n.priority)
        );

         queue.add(new nodeEnter(fromNode,0));
         ...
         while(!queue.isEmpty()){
             ...
             for(var edge : current.getEdges()){
                 ...
                 if(newDistance < distances.get(edge.to)){
                     ...
                     queue.add(new nodeEnter(edge.to,newDistance));
             }
         }
        }
    ...
    private class nodeEntry{
        ...
        public nodeEntry(Node node,Integer priority){
            this.node = node;
            this.priority = priority;
        }
    }
    public int getShortDistance(String from,String to){
        var fromNode = nodes.get(from);
        Map<Node,Integer> distances = new HashMap<>();
        for(var node : nodes.values())
            distances.put(node,Integer.MAX_VALUE);
    ...
    private class entries {
        ...
        public entries(Node node, Integer priority) {
            this.node = node;
            this.priority = priority;
        }
    }
